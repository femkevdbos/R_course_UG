...
- Base category
- Cannot include all categorical variables
...
- If X and Z are highly correlated -> difficult to distinguish effect of X and Z
...
## Interpreting Results
- Why is diet 1 not included?
- What is the effect of time increasing by 1?
- What is the difference between Diet 1 and 2 at time = 0?
- What is the difference in the effect of time between Diet 1 and diet 3?

# Logit

## Introduction to the Logit Model
- Binary dependent variable
- Use the logistic function to estimate $P(Y = 1 ∣ X=x)$
- In the linear regression we estimate $E(Y ∣ X=x)$
...
## Note: Multiplicative Effects
- Be cautious when interpreting results!
- There is an important difference between a linear model and logit model in how to interpret coefficients
- As seen, the exponent of the coefficients represent the change in the odds ratio for a one-unit increase in the variable
- So, in both binary and multinomial logistic regression: the effect of predictor variables is multiplicative, meaning they affect the odds of an outcome category, rather than having a simple additive effect on the probability
...
Note: we focus on non-parametric bootstrapping in this class
...
# Many More Models!
## Additonal Models 
- Parametric / semi-parametric bootstrapping
- Instrumental variable models: use `ivreg`
- Time series models: 
  - Use `ts` package
  - ARIMA models: use `auto.arima` or `Arima`
- Panel data models: use `plm` for fixed effect models for example (DiD)
- Count data models: use `glm` package and poisson or neg. binomial function
